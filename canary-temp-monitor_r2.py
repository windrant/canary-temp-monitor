...
def prepare_data(data,interval):
    ...
    datetime_string = onering.get_current_datetime()
    return datetime_string, avg_temp, avg_humid
...
if __name__ == "__main__":
    ...
    slack_log_freq, slack_contacts, sms_contacts, key = load_settings(SETTINGS_PATH)
    POLL = 59 #seconds
    ...
    display = f"{onering.get_current_datetime()} :: Canary monitoring begins, {quota_remaining} sms in quota remaining, logs: http://{ip}/start"
    ...
    while True:
        ...
        results=sensor_reading(sensor) #Get data from sensor
        ...
        if timer % QUARTERHOUR == 0:
            ...
            alarm_status = alarm_check(quarter_data, temp_max, temp_min, humid_max, humid_min)
            display = f"{quarter_data[0]} :: {quarter_data[1]}F {quarter_data[2]}% {alarm_status[1]}"
            wiki_display = "  * " + display + "\n"
            onering.add_line(wiki_display, log_file)
            if alarm_status[0] == 1:
        ...
        if timer % HOUR == 0:
            if slack_log_freq == "1hour":
                ...
                display = f"One Hour Report: {hour_data[0]} :: {hour_data[1]}F {hour_data[2]}% {alarm_status[1]}"
                notify(display, sms_contacts, slack_contacts, "log")
        if timer % SIXHOUR == 0:
            if slack_log_freq == "6hour":
                six_hour_data = prepare_data(sensor_data,SIXHOUR)
                display = f"Six Hour Report: {six_hour_data[0]} :: {six_hour_data[1]}F {six_hour_data[2]}% {alarm_status[1]}"
                notify(display, sms_contacts, slack_contacts, "log")
        if timer % TWELVEHOUR == 0:
            if slack_log_freq == "12hour":
                twelve_hour_data = prepare_data(sensor_data,HOUR)
                display = f"Twelve Hour Report: {twelve_hour_data[0]} :: {twelve_hour_data[1]}F {twelve_hour_data[2]}% {alarm_status[1]}"
                notify(display, sms_contacts, slack_contacts, "log")
        if timer % DAY == 0:
            timer = 0
            sensordata = []
